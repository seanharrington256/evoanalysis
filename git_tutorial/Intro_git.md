...
- [Configuring Github on MedicineBow](#Configuring-Github-on-MedicineBow)
- [Git on MedicineBow](#Git-on-MedicineBow)
...
I use Git + Github for code for all of my research papers, both to keep the code tracked and to share the code when publishing manuscripts. E.g., a paper I published in 2024 links to [this Github repo](https://github.com/seanharrington256/Coexp_scripts) so that anyone can see my code and replicate my analyses.
...
In "Repository name" enter `evoanalysis2026`. This will obviously be the name of the repository, but will also be part of the web address to the Github page for the repository as well as the name of the directory for the git repository when you clone it to your own computer, MedicineBow, or another server.

In `Description`, you can enter something like "Repo for 2026 Advanced Evolutionary Analysis course". This should be relatively brief, more detail about the repository and files it contains will go into the Readme file. You can also skip a description entirely if you'd like.
...
We can leave the rest of the options at defaults and click `Create repository`. You should now have an empty Github repository. We’ll use Git on MedicineBow to add the contents of this repository.
...
## Configuring Github on MedicineBow
...
Then it will ask you for a passphrase, which you can leave empty by just hitting enter both times when prompted. I won't use a passphrase, but you can if you want; just make sure to remember it, as you’ll be asked for it any time you use the key.

Now you need to go copy the public key so that you can associate it with your Github account. I use `less ~/.ssh/id_rsa.pub` to look at the key and copy it (`q` quits less). Make sure you’re copying the **public key**, ending in .pub, not the private key.
...
Now that Github is set up, we'll start using Git on MedicineBow. Git is already installed on MedicineBow and should also come installed on Mac and Linux operating systems, but will [need to be installed on Windows](https://gitforwindows.org/) (unless this has changed recently, which is possible).

We'll need to start by configuring MedicineBow to use the ssh key that we linked to Github:
...
Since we have already created the repo on Github, the easiest way to link up that repo to MedicineBow is to simply clone the repository. Go to the Github page for your `evoanalysis2026` repository, and you should see something that looks like this, but with your username instead of mine:
...
You can then just copy that address and **on MedicineBow** run:

```
git clone git@github.com:YOURUSERNAME/evoanalysis2026.git
```

Replacing the address after `git clone` with what you copied from Github.

You should now see a new directory on MedicineBow called `evoanalysis2026` that is linked to Github.
...
Alternately, instead of cloning, you can do all of this linking and setup from the command line on MedicineBow. This follows the instructions that you'll see on Github just below the address we copied and pasted above.

I'm going to delete the current empty repo that we just cloned first:

```
rm -r evoanalysis2026
```

Then create a new directory with the same name (it doesn't actually need to have the same name), move into it, and initialize it as a git repo:
...
```
echo "# evoanalysis2026" >> README.md # put text into new file "README.md"
...
```
git branch -M main
git remote add origin git@github.com:YOURUSERNAME/evoanalysis2026.git
...
or just add any changes to all contents of the current directory/repository (remember that `.` means "current directory"):
...
To copy your repository to a new computer, all you need to do is follow the steps above that we used to link MedicineBow to Github (ssh keys, config file) and then follow the same step to clone the repository.

If you have multiple repository copies, you will need to make sure that changes you made on another computer are synced to whatever repository you are currently working on. E.g., if you make some changes to your repository on MedicineBow, push those changes to Github, and then want to make more changes in a local copy of the repo on your laptop, you'll want to first download the changes from Github onto your laptop.
...
At this point, you might test out you new code and find out after a little while that it is actually broken. In this case, you might want to revert to the previous version.
...
Note that on a shared repository, you would probably want to create a new branch, rather than using this kind of hard reset. We'll talk about branching briefly soon.
